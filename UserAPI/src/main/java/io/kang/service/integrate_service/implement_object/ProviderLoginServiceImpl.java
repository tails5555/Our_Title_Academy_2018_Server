...
public class ProviderLoginServiceImpl implements ProviderLoginService {
    ...
    public UserVO provideLogin(final String loginId, final String password){
        if(loginId == null || loginId.isEmpty())
            throw new CustomException("ID의 값을 입력하지 않았습니다.", HttpStatus.UNPROCESSABLE_ENTITY);
        UserVO userVO = userService.findByLoginId(loginId);
        if(userVO == null)
            throw new CustomException("존재하지 않는 사용자의 ID를 입력하였습니다.", HttpStatus.UNPROCESSABLE_ENTITY);
        else {
            String encryptPassword = Encryption.encrypt(password, Encryption.MD5);
            if(!userVO.getPassword().equals(encryptPassword))
                throw new CustomException("사용자의 비밀번호가 일치하지 않습니다.", HttpStatus.UNPROCESSABLE_ENTITY);
            else
                return userVO;
        }
    }
