...
public enum ProfileCreateSingleton {
    ...
    private Profile profile = new Profile(null, this.user, "PROFILE_FILE_NAME_01", 0, new byte[0], Suffix.PNG, LocalDateTime.now());
    public Profile getInstance(){
        if(this.user == null){
            this.user = UserUpdateSingleton.INSTANCE.getInstance();
        }
        if(this.profile == null){
            return new Profile(null, this.user, "PROFILE_FILE_NAME_01", 0, new byte[0], Suffix.PNG, LocalDateTime.now());
